...
import { getStorage, getDownloadURL, ref as sRef, listAll, uploadBytesResumable } from 'firebase/storage'
...
const config = {
  ...
  databaseURL: "https://my-contact-page-89-default-rtdb.asia-southeast1.firebasedatabase.app",
...
class Firebase {
  ...
  writeEBookDataToDB = (title, eBookDownloadUrl, eBookCoverDownloadUrl) => {
    return set(ref(db, `eBooks/${title}`), { title, eBookDownloadUrl, eBookCoverDownloadUrl })
  }
  ...
  uploadEBookFiles = (file, setProgressLevel) => {
    ...
    const eBookCoverStorageRef = sRef(storage, `ebooks-cover/${file.eBookName}-cover`)

    const uploadeBookTask = uploadBytesResumable(eBookStorageRef, file.eBook)
    uploadeBookTask.on(
      'state_changed',
      (snap) => {
        setProgressLevel(prev => ({ ...prev, ['eBookProgressLevel']: snap.bytesTransferred / snap.totalBytes * 100 }))
      },
      (err) => console.log(err),
      () => {
        getDownloadURL(eBookStorageRef).then((downloadUrl) => {
          ...
          if (eBookCoverDownloadUrl && eBookDownloadUrl) {
            ...
            this.writeEBookDataToDB(file.eBookName, eBookDownloadUrl, eBookCoverDownloadUrl)
    ...
    const uploadeBookCoverTask = uploadBytesResumable(eBookCoverStorageRef, file.eBookCover)
    uploadeBookCoverTask.on(
      'state_changed',
      (snap) => {
        console.log("eBook Cover Progress ->" + snap.bytesTransferred / snap.totalBytes * 100)
        setProgressLevel(prev => ({ ...prev, ['eBookCoverProgressLevel']: snap.bytesTransferred / snap.totalBytes * 100 }))
      },
      (err) => { console.log(err) },
      () => {
  ...
  signInWithGoogle = () => {
    signInWithPopup(auth, googleProvider)
      .then(result => console.log('Signin with Google Successful...'))
